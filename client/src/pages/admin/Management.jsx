...
// APIs
import { deleteAPI, paginationAPI } from "@api/requests/managementApi";

// Modal
import VenManagement from "./components/modals/VenManagement";
...
const Management = () => {
    ...
    const canCreate = hasPermission("managementRefunds", "management", "create");
    const canEdit = hasPermission("managementRefunds", "management", "edit");
    const canDelete = hasPermission("managementRefunds", "management", "delete");
    ...
    const renderActions = (item) => {
        const { gerId, nombre } = item;
        const menuItems = [
            {
                label: "Editar",
                icon: "pi pi-pencil",
                command: () => venInsureTypRef.current.editManagement(item),
                disabled: !canEdit,
                color: "#fda53a",
            },
            {
                ...
                icon: "pi pi-trash",
                command: () => {
                    setCurrentManagement({ gerId, nombre });
        ...
        return <ContextMenuActions menuItems={menuItems} itemId={gerId} />;
    };
    ...
    const actionsToolbar = useMemo(
        () => (
            <>
                ...
                <RightToolbar
                    label="Crear"
                    onClick={() => venInsureTypRef.current.newManagement()}
                    disabled={!canCreate}
                />
            </>
        ),
    ...
    const handleRowSelect = (e) => {
        ...
        venInsureTypRef.current.editManagement(e.value);
    };
    ...
    return firstLoad ? (
        ...
        <div className="fade-in">
            <ConfirmDialog
                ...
                message={`¿Deseas eliminar el registro ${currentManagement?.nombre}?`}
                accept={() => {
                    deleteItemApi(currentManagement?.gerId);
                        ...
                        <DataTableComponentMemo
                            ...
                            emptyMessage={
                                <EmptyState
                                    ...
                                    onButtonClick={() => venInsureTypRef.current.newManagement()}
                                    canCreate={canCreate}
                                />
                            }
                        />
