...
// APIs
import { deleteAPI, paginationAPI } from "@api/requests/centerCostApi";

// Modal
import VenCenterCost from "./components/modals/VenCenterCost";
...
const CenterCost = () => {
    ...
    const canCreate = hasPermission("managementRefunds", "centerCost", "create");
    const canEdit = hasPermission("managementRefunds", "centerCost", "edit");
    const canDelete = hasPermission("managementRefunds", "centerCost", "delete");
    ...
    const [currentCenterCost, setCurrentCenterCost] = useState(null);

    const deleteItemApi = useCallback(
        async (id) => {
            try {
                await deleteAPI({ ccoIds: [id], usuario: nombreusuario });
                showSuccess("registro eliminada correctamente");
                venInsureTypRef.current.onClose();
                deleteItem({ id, idField: "ccoId" });
            } catch (error) {
    ...
    const renderActions = (item) => {
        const { ccoId, nombre } = item;
        const menuItems = [
            {
                label: "Editar",
                icon: "pi pi-pencil",
                command: () => venInsureTypRef.current.editCenterCost(item),
                disabled: !canEdit,
                color: "#fda53a",
            },
            {
                ...
                icon: "pi pi-trash",
                command: () => {
                    setCurrentCenterCost({ ccoId, nombre });
        ...
        return <ContextMenuActions menuItems={menuItems} itemId={ccoId} />;
    };
    ...
    const actionsToolbar = useMemo(
        () => (
            <>
                ...
                <RightToolbar
                    label="Crear"
                    onClick={() => venInsureTypRef.current.newCenterCost()}
                    disabled={!canCreate}
                />
            </>
        ),
    ...
    const handleRowSelect = (e) => {
        ...
        venInsureTypRef.current.editCenterCost(e.value);
    };
    ...
    return firstLoad ? (
        ...
        <div className="fade-in">
            <ConfirmDialog
                ...
                message={`¿Deseas eliminar el registro ${currentCenterCost?.nombre}?`}
                accept={() => {
                    deleteItemApi(currentCenterCost?.ccoId);
            ...
            <VenCenterCost
                ...
                setCurrentCenterCost={setCurrentCenterCost}
                canDelete={canDelete}
            />
                        ...
                        <DataTableComponentMemo
                            ...
                            emptyMessage={
                                <EmptyState
                                    ...
                                    onButtonClick={() => venInsureTypRef.current.newCenterCost()}
                                    canCreate={canCreate}
                                />
                            }
                        />
