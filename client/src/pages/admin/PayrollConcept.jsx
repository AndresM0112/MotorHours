...
// APIs
import { deleteAPI, paginationAPI } from "@api/requests/payrollConceptApi";

// Modal
import VenPayrollConcept from "./components/modals/VenPayrollConcept";
...
const PayrollConcept = () => {
    const VenPayrollConceptRef = useRef(null);
    ...
    const canCreate = hasPermission("managementRefunds", "payrollConcept", "create");
    const canEdit = hasPermission("managementRefunds", "payrollConcept", "edit");
    const canDelete = hasPermission("managementRefunds", "payrollConcept", "delete");
    ...
    const [currentPayrollConcept, setCurrentPayrollConcept] = useState(null);

    const deleteItemApi = useCallback(
        async (id) => {
            try {
                await deleteAPI({ conIds: [id], usuario: nombreusuario });
                showSuccess("tipo concepto de nomina eliminada correctamente");
                VenPayrollConceptRef.current.onClose();
                deleteItem({ id, idField: "conId" });
            } catch (error) {
    ...
    const renderActions = (item) => {
        const { conId, nombre } = item;
        const menuItems = [
            {
                label: "Editar",
                icon: "pi pi-pencil",
                command: () => VenPayrollConceptRef.current.editPayrollConcept(item),
                disabled: !canEdit,
                color: "#fda53a",
            },
            {
                ...
                icon: "pi pi-trash",
                command: () => {
                    setCurrentPayrollConcept({ conId, nombre });
        ...
        return <ContextMenuActions menuItems={menuItems} itemId={conId} />;
    };
    ...
    const columnsConfig = [
        ...
        {
            field: "tipoConceptoNombre",
            header: "Tipo de Concepto",
            style: { minWidth: "10rem" },
        },
        {
            field: "naturalezaNombre",
            header: "Naturaleza",
            style: { minWidth: "10rem" },
        },
        {
            ...
            body: ({ factor }) => parseFloat(factor).toFixed(2),
        },
        {
            field: "fueraNomina",
            header: "Fuera Nómina",
            style: { minWidth: "8rem", textAlign: "center" },
            body: ({ fueraNomina }) => (fueraNomina === 1 ? "Sí" : fueraNomina === 2 ? "No" : "No aplica"),
        },
        {
            field: "requiereFondo",
            header: "Req. Fondo",
            style: { minWidth: "8rem", textAlign: "center" },
            body: ({ requiereFondo }) => (requiereFondo === 1 ? "Sí" : requiereFondo === 2 ? "No" : "No aplica"),
        },
        {
            field: "predeterminado",
            header: "Predet.",
            style: { minWidth: "8rem", textAlign: "center" },
            body: ({ predeterminado }) => (predeterminado === 1 ? "Sí" : predeterminado === 2 ? "No" : "No aplica"),
        },
        {
            field: "aplica",
            header: "Aplica para informe",
            style: { minWidth: "8rem", textAlign: "center" },
            body: ({ aplica }) => (aplica === 1 ? "Sí" : "No"),
        },
        {
            field: "estadoNombre",
            header: "Estado",
            style: { maxWidth: "8rem" },
            body: ({ estId, estadoNombre }) => (
                <ChipStatusComponent
                    id={estId}
                    nameStatus={estadoNombre}
    ...
    const actionsToolbar = useMemo(
        () => (
            <>
                ...
                <RightToolbar
                    label="Crear"
                    onClick={() => VenPayrollConceptRef.current.newPayrollConcept()}
                    disabled={!canCreate}
                />
            </>
        ),
    ...
    const handleRowSelect = (e) => {
        ...
        VenPayrollConceptRef.current.editPayrollConcept(e.value);
    };
    ...
    return firstLoad ? (
        ...
        <div className="fade-in">
            <ConfirmDialog
                ...
                message={`¿Deseas eliminar la tipo concepto de nomina ${currentPayrollConcept?.nombre}?`}
                accept={() => {
                    deleteItemApi(currentPayrollConcept?.conId);
            ...
            <VenPayrollConcept
                ref={VenPayrollConceptRef}
                addItem={addItem}
                updateItem={updateItem}
                setDeleteDialogVisible={setDeleteDialogVisible}
                setCurrentPayrollConcept={setCurrentPayrollConcept}
                canDelete={canDelete}
            />
                ...
                <div className="grid">
                    <div className="col-12">
                        <DataTableComponentMemo
                            KeyModule="module_payroll_concept"
                            dataKey="conId"
                            ...
                            emptyMessage={
                                <EmptyState
                                    ...
                                    onButtonClick={() =>
                                        VenPayrollConceptRef.current.newPayrollConcept()
                                    }
                                    canCreate={canCreate}
                                />
                            }
