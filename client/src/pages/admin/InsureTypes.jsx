...
// APIs
import { deleteAPI, paginationAPI } from "@api/requests/insureTypeApi";

// Modal
import VenInsureType from "./components/modals/VenInsureType";
...
const InsureTypes = () => {
    ...
    const canCreate = hasPermission("managementRefunds", "insureTypes", "create");
    const canEdit = hasPermission("managementRefunds", "insureTypes", "edit");
    const canDelete = hasPermission("managementRefunds", "insureTypes", "delete");
    ...
    const [currentInsureType, setCurrentInsureType] = useState(null);

    const deleteItemApi = useCallback(
        async (id) => {
            try {
                await deleteAPI({ tiaIds: [id], usuario: nombreusuario });
                showSuccess("tipo de aseguradora eliminada correctamente");
                venInsureTypRef.current.onClose();
                deleteItem({ id, idField: "tiaId" });
            } catch (error) {
    ...
    const renderActions = (item) => {
        const { tiaId, nombre } = item;
        const menuItems = [
            {
                label: "Editar",
                icon: "pi pi-pencil",
                command: () => venInsureTypRef.current.editInsureType(item),
                disabled: !canEdit,
                color: "#fda53a",
            },
            {
                ...
                icon: "pi pi-trash",
                command: () => {
                    setCurrentInsureType({ tiaId, nombre });
        ...
        return <ContextMenuActions menuItems={menuItems} itemId={tiaId} />;
    };
    ...
    const actionsToolbar = useMemo(
        () => (
            <>
                ...
                <RightToolbar
                    label="Crear"
                    onClick={() => venInsureTypRef.current.newInsureType()}
                    disabled={!canCreate}
                />
            </>
        ),
    ...
    const handleRowSelect = (e) => {
        ...
        venInsureTypRef.current.editInsureType(e.value);
    };
    ...
    return firstLoad ? (
        ...
        <div className="fade-in">
            <ConfirmDialog
                ...
                message={`¿Deseas eliminar el tipo de aseguradora ${currentInsureType?.nombre}?`}
                accept={() => {
                    deleteItemApi(currentInsureType?.tiaId);
            ...
            <VenInsureType
                ...
                setCurrentInsureType={setCurrentInsureType}
                canDelete={canDelete}
            />
                        ...
                        <DataTableComponentMemo
                            ...
                            emptyMessage={
                                <EmptyState
                                    ...
                                    description="Puedes crear una nueva tipo de aseguradora para comenzar."
                                    buttonLabel="Registrar nueva tipo de aseguradora"
                                    onButtonClick={() => venInsureTypRef.current.newInsureType()}
                                    canCreate={canCreate}
                                />
                            }
                        />
